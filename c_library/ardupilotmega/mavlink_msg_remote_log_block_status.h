...
//----------------------------------------
//-- Message REMOTE_LOG_BLOCK_STATUS pack,encode routines, for sending
//----------------------------------------
    ...
    fmav_status_t* _status)
{
    ...
    _msg->payload_max_len = FASTMAVLINK_MSG_REMOTE_LOG_BLOCK_STATUS_PAYLOAD_LEN_MAX;
...
//----------------------------------------
//-- Message REMOTE_LOG_BLOCK_STATUS decode routines, for receiving
//----------------------------------------
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_remote_log_block_status_decode(fmav_remote_log_block_status_t* payload, const fmav_message_t* msg)
{
    ...
    if (msg->len < FASTMAVLINK_MSG_REMOTE_LOG_BLOCK_STATUS_PAYLOAD_LEN_MAX) {
        ...
        memset(&(((uint8_t*)payload)[msg->len]), 0, FASTMAVLINK_MSG_REMOTE_LOG_BLOCK_STATUS_PAYLOAD_LEN_MAX - msg->len);
    } else {
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_remote_log_block_status_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_remote_log_block_status_t* _payload)
{
    return mavlink_msg_remote_log_block_status_pack(
        sysid,
        compid,
        _msg,
        _payload->target_system, _payload->target_component, _payload->seqno, _payload->status);
