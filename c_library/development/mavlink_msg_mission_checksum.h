...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_mission_checksum_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_mission_checksum_t* _payload)
{
    return mavlink_msg_mission_checksum_pack(
        sysid,
        compid,
        _msg,
        _payload->mission_type, _payload->checksum);
