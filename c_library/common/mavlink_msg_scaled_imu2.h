...
// while for the fmav_msg_scaled_imu2_decode() function, this could be accounted for,
// there is no easy&reasonable way to do it for the fmav_msg_scaled_imu2_get_field_yyy() functions.
// So, we generally require it.

// this should not be needed, but we provide it just in case
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_scaled_imu2_zero_fill(fmav_message_t* msg)
{
    if (msg->len < FASTMAVLINK_MSG_SCALED_IMU2_PAYLOAD_LEN_MAX) {
        memset(&(((uint8_t*)msg->payload)[msg->len]), 0, FASTMAVLINK_MSG_SCALED_IMU2_PAYLOAD_LEN_MAX - msg->len); // zero-fill
    }
}
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_scaled_imu2_decode(fmav_scaled_imu2_t* payload, const fmav_message_t* msg)
{
    ...
    if (msg->len < FASTMAVLINK_MSG_SCALED_IMU2_PAYLOAD_LEN_MAX) {
        ...
        memset(&(((uint8_t*)payload)[msg->len]), 0, FASTMAVLINK_MSG_SCALED_IMU2_PAYLOAD_LEN_MAX - msg->len); // zero-fill
    } else {
        memcpy(payload, msg->payload, FASTMAVLINK_MSG_SCALED_IMU2_PAYLOAD_LEN_MAX);
    }
}
