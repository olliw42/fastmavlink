...
#define FASTMAVLINK_MSG_REMOTE_LOG_BLOCK_STATUS_FRAME_LEN_MAX  32



#define FASTMAVLINK_MSG_REMOTE_LOG_BLOCK_STATUS_FIELD_SEQNO_OFS  0
#define FASTMAVLINK_MSG_REMOTE_LOG_BLOCK_STATUS_FIELD_TARGET_SYSTEM_OFS  4
#define FASTMAVLINK_MSG_REMOTE_LOG_BLOCK_STATUS_FIELD_TARGET_COMPONENT_OFS  5
#define FASTMAVLINK_MSG_REMOTE_LOG_BLOCK_STATUS_FIELD_STATUS_OFS  6
...
// while for the fmav_msg_remote_log_block_status_decode() function, this could be accounted for,
// there is no easy&reasonable way to do it for the fmav_msg_remote_log_block_status_get_field_yyy() functions.
// So, we generally require it.

// this should not be needed, but we provide it just in case
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_remote_log_block_status_zero_fill(fmav_message_t* msg)
{
    if (msg->len < FASTMAVLINK_MSG_REMOTE_LOG_BLOCK_STATUS_PAYLOAD_LEN_MAX) {
        memset(&(((uint8_t*)msg->payload)[msg->len]), 0, FASTMAVLINK_MSG_REMOTE_LOG_BLOCK_STATUS_PAYLOAD_LEN_MAX - msg->len); // zero-fill
    }
}
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_remote_log_block_status_decode(fmav_remote_log_block_status_t* payload, const fmav_message_t* msg)
{
    ...
    //memcpy(payload, msg->payload, FASTMAVLINK_MSG_REMOTE_LOG_BLOCK_STATUS_PAYLOAD_LEN_MAX);

    // let's assume it is not zero filled, this should not be needed, but let's just play it safe
    if (msg->len < FASTMAVLINK_MSG_REMOTE_LOG_BLOCK_STATUS_PAYLOAD_LEN_MAX) {
        memcpy(payload, msg->payload, msg->len);
        memset(&(((uint8_t*)payload)[msg->len]), 0, FASTMAVLINK_MSG_REMOTE_LOG_BLOCK_STATUS_PAYLOAD_LEN_MAX - msg->len); // zero-fill
    } else {
        memcpy(payload, msg->payload, FASTMAVLINK_MSG_REMOTE_LOG_BLOCK_STATUS_PAYLOAD_LEN_MAX);
    }
}
