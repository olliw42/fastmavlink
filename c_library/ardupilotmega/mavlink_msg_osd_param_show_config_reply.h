...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_osd_param_show_config_reply_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_osd_param_show_config_reply_t* _payload)
{
    return mavlink_msg_osd_param_show_config_reply_pack(
        sysid,
        compid,
        _msg,
        _payload->request_id, _payload->result, _payload->param_id, _payload->config_type, _payload->min_value, _payload->max_value, _payload->increment);
