...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_compassmot_status_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_compassmot_status_t* _payload)
{
    return mavlink_msg_compassmot_status_pack(
        sysid,
        compid,
        _msg,
        _payload->throttle, _payload->current, _payload->interference, _payload->CompensationX, _payload->CompensationY, _payload->CompensationZ);
