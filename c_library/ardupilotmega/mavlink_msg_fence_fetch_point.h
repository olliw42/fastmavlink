...
//----------------------------------------
//-- Message FENCE_FETCH_POINT pack,encode routines, for sending
//----------------------------------------
    ...
    fmav_status_t* _status)
{
    ...
    _msg->payload_max_len = FASTMAVLINK_MSG_FENCE_FETCH_POINT_PAYLOAD_LEN_MAX;
...
//----------------------------------------
//-- Message FENCE_FETCH_POINT decode routines, for receiving
//----------------------------------------
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_fence_fetch_point_decode(fmav_fence_fetch_point_t* payload, const fmav_message_t* msg)
{
    ...
    if (msg->len < FASTMAVLINK_MSG_FENCE_FETCH_POINT_PAYLOAD_LEN_MAX) {
        ...
        memset(&(((uint8_t*)payload)[msg->len]), 0, FASTMAVLINK_MSG_FENCE_FETCH_POINT_PAYLOAD_LEN_MAX - msg->len);
    } else {
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_fence_fetch_point_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_fence_fetch_point_t* _payload)
{
