...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_autopilot_version_request_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_autopilot_version_request_t* _payload)
{
    return mavlink_msg_autopilot_version_request_pack(
        sysid,
        compid,
        _msg,
        _payload->target_system, _payload->target_component);
