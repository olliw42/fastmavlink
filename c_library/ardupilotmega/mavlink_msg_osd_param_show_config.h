...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_osd_param_show_config_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_osd_param_show_config_t* _payload)
{
    return mavlink_msg_osd_param_show_config_pack(
        sysid,
        compid,
        _msg,
        _payload->target_system, _payload->target_component, _payload->request_id, _payload->osd_screen, _payload->osd_index);
