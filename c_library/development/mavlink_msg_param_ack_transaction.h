...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_param_ack_transaction_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_param_ack_transaction_t* _payload)
{
    return mavlink_msg_param_ack_transaction_pack(
        sysid,
        compid,
        _msg,
        _payload->target_system, _payload->target_component, _payload->param_id, _payload->param_value, _payload->param_type, _payload->param_result);
