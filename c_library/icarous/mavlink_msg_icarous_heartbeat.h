...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_icarous_heartbeat_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_icarous_heartbeat_t* _payload)
{
    return mavlink_msg_icarous_heartbeat_pack(
        sysid,
        compid,
        _msg,
        _payload->status);
