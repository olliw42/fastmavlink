...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_device_op_read_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_device_op_read_t* _payload)
{
    return mavlink_msg_device_op_read_pack(
        sysid,
        compid,
        _msg,
        _payload->target_system, _payload->target_component, _payload->request_id, _payload->bustype, _payload->bus, _payload->address, _payload->busname, _payload->regstart, _payload->count, _payload->bank);
