...
#define FASTMAVLINK_MSG_FENCE_FETCH_POINT_FRAME_LEN_MAX  28



#define FASTMAVLINK_MSG_FENCE_FETCH_POINT_FIELD_TARGET_SYSTEM_OFS  0
#define FASTMAVLINK_MSG_FENCE_FETCH_POINT_FIELD_TARGET_COMPONENT_OFS  1
#define FASTMAVLINK_MSG_FENCE_FETCH_POINT_FIELD_IDX_OFS  2
...
{
    fmav_fence_fetch_point_t* _payload = (fmav_fence_fetch_point_t*)_msg->payload;
    ...
    _msg->sysid = sysid;
    _msg->compid = compid;
    _msg->msgid = FASTMAVLINK_MSG_ID_FENCE_FETCH_POINT;
    _msg->target_sysid = target_system;
    _msg->target_compid = target_component;
    _msg->crc_extra = FASTMAVLINK_MSG_FENCE_FETCH_POINT_CRCEXTRA;
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t fmav_msg_fence_fetch_point_pack_to_frame_buf(
    ...
    fmav_status_t* _status)
{
    fmav_fence_fetch_point_t* _payload = (fmav_fence_fetch_point_t*)(&_buf[FASTMAVLINK_HEADER_V2_LEN]);
    ...
    _buf[5] = sysid;
    _buf[6] = compid;
    _buf[7] = (uint8_t)FASTMAVLINK_MSG_ID_FENCE_FETCH_POINT;
    _buf[8] = ((uint32_t)FASTMAVLINK_MSG_ID_FENCE_FETCH_POINT >> 8);
    _buf[9] = ((uint32_t)FASTMAVLINK_MSG_ID_FENCE_FETCH_POINT >> 16);
...
// while for the fmav_msg_fence_fetch_point_decode() function, this could be accounted for,
// there is no easy&reasonable way to do it for the fmav_msg_fence_fetch_point_get_field_yyy() functions.
// So, we generally require it.

// this should not be needed, but we provide it just in case
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_fence_fetch_point_zero_fill(fmav_message_t* msg)
{
    if (msg->len < FASTMAVLINK_MSG_FENCE_FETCH_POINT_PAYLOAD_LEN_MAX) {
        memset(&(((uint8_t*)msg->payload)[msg->len]), 0, FASTMAVLINK_MSG_FENCE_FETCH_POINT_PAYLOAD_LEN_MAX - msg->len); // zero-fill
    }
}
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_fence_fetch_point_decode(fmav_fence_fetch_point_t* payload, const fmav_message_t* msg)
{
    ...
    //memcpy(payload, msg->payload, FASTMAVLINK_MSG_FENCE_FETCH_POINT_PAYLOAD_LEN_MAX);

    // let's assume it is not zero filled, this should not be needed, but let's just play it safe
    if (msg->len < FASTMAVLINK_MSG_FENCE_FETCH_POINT_PAYLOAD_LEN_MAX) {
        memcpy(payload, msg->payload, msg->len);
        memset(&(((uint8_t*)payload)[msg->len]), 0, FASTMAVLINK_MSG_FENCE_FETCH_POINT_PAYLOAD_LEN_MAX - msg->len); // zero-fill
    } else {
        memcpy(payload, msg->payload, FASTMAVLINK_MSG_FENCE_FETCH_POINT_PAYLOAD_LEN_MAX);
    }
}
...
FASTMAVLINK_FUNCTION_DECORATOR uint8_t fmav_msg_fence_fetch_point_get_field_target_system(const fmav_message_t* msg)
{
    uint8_t r;
...
FASTMAVLINK_FUNCTION_DECORATOR uint8_t fmav_msg_fence_fetch_point_get_field_target_component(const fmav_message_t* msg)
{
    uint8_t r;
...
FASTMAVLINK_FUNCTION_DECORATOR uint8_t fmav_msg_fence_fetch_point_get_field_idx(const fmav_message_t* msg)
{
    uint8_t r;
