...
{
    fmav_esc_telemetry_1_to_4_t* _payload = (fmav_esc_telemetry_1_to_4_t*)_msg->payload;
    ...
    _msg->sysid = sysid;
    _msg->compid = compid;
    _msg->msgid = FASTMAVLINK_MSG_ID_ESC_TELEMETRY_1_TO_4;
    _msg->target_sysid = 0;
    _msg->target_compid = 0;
    _msg->crc_extra = FASTMAVLINK_MSG_ESC_TELEMETRY_1_TO_4_CRCEXTRA;
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t fmav_msg_esc_telemetry_1_to_4_pack_to_frame_buf(
    ...
    fmav_status_t* _status)
{
    fmav_esc_telemetry_1_to_4_t* _payload = (fmav_esc_telemetry_1_to_4_t*)(&_buf[FASTMAVLINK_HEADER_V2_LEN]);
    ...
    _buf[5] = sysid;
    _buf[6] = compid;
    _buf[7] = (uint8_t)FASTMAVLINK_MSG_ID_ESC_TELEMETRY_1_TO_4;
    _buf[8] = ((uint32_t)FASTMAVLINK_MSG_ID_ESC_TELEMETRY_1_TO_4 >> 8);
    _buf[9] = ((uint32_t)FASTMAVLINK_MSG_ID_ESC_TELEMETRY_1_TO_4 >> 16);
...
// while for the fmav_msg_esc_telemetry_1_to_4_decode() function, this could be accounted for,
// there is no easy&reasonable way to do it for the fmav_msg_esc_telemetry_1_to_4_get_field_yyy() functions.
// So, we generally require it.

// this should not be needed, but we provide it just in case
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_esc_telemetry_1_to_4_zero_fill(fmav_message_t* msg)
{
    if (msg->len < FASTMAVLINK_MSG_ESC_TELEMETRY_1_TO_4_PAYLOAD_LEN_MAX) {
        memset(&(((uint8_t*)msg->payload)[msg->len]), 0, FASTMAVLINK_MSG_ESC_TELEMETRY_1_TO_4_PAYLOAD_LEN_MAX - msg->len); // zero-fill
    }
}
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_esc_telemetry_1_to_4_decode(fmav_esc_telemetry_1_to_4_t* payload, const fmav_message_t* msg)
{
    ...
    if (msg->len < FASTMAVLINK_MSG_ESC_TELEMETRY_1_TO_4_PAYLOAD_LEN_MAX) {
        ...
        memset(&(((uint8_t*)payload)[msg->len]), 0, FASTMAVLINK_MSG_ESC_TELEMETRY_1_TO_4_PAYLOAD_LEN_MAX - msg->len); // zero-fill
    } else {
        memcpy(payload, msg->payload, FASTMAVLINK_MSG_ESC_TELEMETRY_1_TO_4_PAYLOAD_LEN_MAX);
    }
}
