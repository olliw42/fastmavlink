...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_device_op_write_reply_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_device_op_write_reply_t* _payload)
{
    return mavlink_msg_device_op_write_reply_pack(
        sysid,
        compid,
        _msg,
        _payload->request_id, _payload->result);
