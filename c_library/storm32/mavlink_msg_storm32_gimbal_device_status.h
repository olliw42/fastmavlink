...
{
    fmav_storm32_gimbal_device_status_t* _payload = (fmav_storm32_gimbal_device_status_t*)_msg->payload;
    ...
    _msg->sysid = sysid;
    _msg->compid = compid;
    _msg->msgid = FASTMAVLINK_MSG_ID_STORM32_GIMBAL_DEVICE_STATUS;
    _msg->target_sysid = target_system;
    _msg->target_compid = target_component;
    _msg->crc_extra = FASTMAVLINK_MSG_STORM32_GIMBAL_DEVICE_STATUS_CRCEXTRA;
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t fmav_msg_storm32_gimbal_device_status_pack_to_frame_buf(
    ...
    fmav_status_t* _status)
{
    fmav_storm32_gimbal_device_status_t* _payload = (fmav_storm32_gimbal_device_status_t*)(&_buf[FASTMAVLINK_HEADER_V2_LEN]);
    ...
    _buf[5] = sysid;
    _buf[6] = compid;
    _buf[7] = (uint8_t)FASTMAVLINK_MSG_ID_STORM32_GIMBAL_DEVICE_STATUS;
    _buf[8] = ((uint32_t)FASTMAVLINK_MSG_ID_STORM32_GIMBAL_DEVICE_STATUS >> 8);
    _buf[9] = ((uint32_t)FASTMAVLINK_MSG_ID_STORM32_GIMBAL_DEVICE_STATUS >> 16);
...
// while for the fmav_msg_storm32_gimbal_device_status_decode() function, this could be accounted for,
// there is no easy&reasonable way to do it for the fmav_msg_storm32_gimbal_device_status_get_field_yyy() functions.
// So, we generally require it.

// this should not be needed, but we provide it just in case
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_storm32_gimbal_device_status_zero_fill(fmav_message_t* msg)
{
    if (msg->len < FASTMAVLINK_MSG_STORM32_GIMBAL_DEVICE_STATUS_PAYLOAD_LEN_MAX) {
        memset(&(((uint8_t*)msg->payload)[msg->len]), 0, FASTMAVLINK_MSG_STORM32_GIMBAL_DEVICE_STATUS_PAYLOAD_LEN_MAX - msg->len); // zero-fill
    }
}
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_storm32_gimbal_device_status_decode(fmav_storm32_gimbal_device_status_t* payload, const fmav_message_t* msg)
{
    ...
    //memcpy(payload, msg->payload, FASTMAVLINK_MSG_STORM32_GIMBAL_DEVICE_STATUS_PAYLOAD_LEN_MAX);

    // let's assume it is not zero filled, this should not be needed, but let's just play it safe
    if (msg->len < FASTMAVLINK_MSG_STORM32_GIMBAL_DEVICE_STATUS_PAYLOAD_LEN_MAX) {
        memcpy(payload, msg->payload, msg->len);
        memset(&(((uint8_t*)payload)[msg->len]), 0, FASTMAVLINK_MSG_STORM32_GIMBAL_DEVICE_STATUS_PAYLOAD_LEN_MAX - msg->len); // zero-fill
    } else {
        memcpy(payload, msg->payload, FASTMAVLINK_MSG_STORM32_GIMBAL_DEVICE_STATUS_PAYLOAD_LEN_MAX);
    }
}
